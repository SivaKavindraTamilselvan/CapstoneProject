...
@Component({
  selector: 'app-update-rejected-variant',
  imports: [DecimalPipe],
  templateUrl: './update-rejected-variant.html',
  styleUrl: './update-rejected-variant.css',
})
export class UpdateRejectedVariant {
  ...
  updateProductModel = signal(new UpdateRejectedProductVariantModel());
  successMessage = signal<string | null>(null);
  errorMessage = signal<string | null>(null);
  updateCategoryId = signal<number | null>(null);
  updateSubCategoryId = signal<number | null>(null);

  showUpdatePopup = signal<boolean>(false);
  editingSku = signal<string>('');
...
// Call this from your table row's "Edit" button
openUpdatePopup(variant: VendorProductVariantModel): void {
  this.successMessage.set(null);
  this.errorMessage.set(null);
  this.editingSku.set(variant.sku);

   const statusId = this.statusOptions.find(
    s => s.label === variant.productVariantStatus
  )?.id ?? 0;

  this.updateProductModel.set(new UpdateRejectedProductVariantModel(
    ...
    variant.widthInCm,
    variant.heightInCm,
    variant.minimuQuantityPerUser,  // number — position 8
    variant.isReturn,               // boolean — position 9
    variant.isExchange              // boolean — position 10
  ));
...
// Generic numeric/string field handler
onUpdateFieldChange(field: keyof UpdateRejectedProductVariantModel, event: Event): void {
  const raw = (event.target as HTMLInputElement).value;
  const val = raw ? Number(raw) : 0;
  this.updateProductModel.update(m => ({ ...m, [field]: val }));
}

onUpdateStatusChange(event: Event): void {
  const val = Number((event.target as HTMLSelectElement).value);
  this.updateProductModel.update(m => ({ ...m, productVariantStatusId: val }));
}

onUpdateBooleanChange(field: 'isReturn' | 'isExchange', event: Event): void {
  const checked = (event.target as HTMLInputElement).checked;
  this.updateProductModel.update(m => ({ ...m, [field]: checked }));
}
...
submitUpdate(): void {
  ...
  this.vendorProductService.updateRejectedVariant(this.updateProductModel()).subscribe({
    next: () => {
      this.isSubmitting.set(false);
      this.successMessage.set('Variant updated successfully.');
      this.loadProductVariant();
      setTimeout(() => this.closeUpdatePopup(), 1500);
    },
    error: (err) => {
      this.isSubmitting.set(false);
