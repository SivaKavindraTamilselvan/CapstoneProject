...
public partial class AdminProductService : IAdminProductService
{
    public async Task<ResponseReviewOfProductDTO> DeleteProduct(RequestDeleteProductDTO requestDeleteProductDTO, int adminUserId)
    {
        ...
        try
        {
            ...
            {
                _logger.LogWarning("ProductId {ProductId} deletion rejected. Already Deleted_By_Admin", product.ProductId);
            ...
            foreach (var item in ownerVendor)
            {
                ...
                _logger.LogInformation("Product delete notification sent to Vendor Owner UserId {UserId} for ProductId {ProductId}", item.UserId, updatedProduct.ProductId);
            }
    ...
    public async Task<ResponseReviewOfProductVariantDTO> DeleteProductVaraint(RequestDeleteVariantDTO requestDeleteProductDTO, int adminUserId)
    {
        ...
        try
        {
            _logger.LogInformation("Admin UserId {AdminUserId} initiated deletion of ProductVariantId {ProductVariantId}", adminUserId, requestDeleteProductDTO.ProductVariantId);

            var product = await _productValidation.AdminValidateProductVariant(requestDeleteProductDTO.ProductVariantId);
            // to check if the product is already deleted. prevent from deleting already deleted product
            if (product.ProductApprovalStatusId == (int)ApprovalStatusEnum.Deleted_By_Admin)
            {
                _logger.LogWarning("ProductVariantId {ProductVariantId} deletion rejected. Already Deleted_By_Admin", product.ProductVariantId);
                throw new DataApprovalStatusException("Product Already Deleted By Admin");
            }
            // validate admin user based on the product admin and check if the admin is active or not
            var adminUser = await _adminUserValidation.ValidateProductAdminUserByUserId(adminUserId);
            _logger.LogInformation("ProductVariant {ProductVariantId} - {SKU} found. Current Approval Status: {StatusId}", product.ProductVariantId, product.SKU, product.ProductApprovalStatusId);
            ...
            _logger.LogInformation("ProductVariant {ProductVariantId} marked as Deleted_By_Admin by AdminUserId {AdminUserId}", product.ProductVariantId, adminUser.AdminUserId);
            ...
            _logger.LogInformation("Approval history created for ProductVariantId {ProductVariantId}", product.ProductVariantId);
            ...
            {
                _logger.LogWarning("Parent ProductId {ProductId} not found while resolving vendor for ProductVariantId {ProductVariantId}. Skipping vendor notification", product.ProductId, product.ProductVariantId);
            }
            else
            {
                var ownerVendor = await _vendorUserRepsository.GetProductVendorUserByVendorId(productDetails.VendorId);
                foreach (var item in ownerVendor)
                {
                    await _notificationService.SendToUser(
                        item.UserId,
                    ...
                    _logger.LogInformation("Product delete notification sent to Vendor Owner UserId {UserId} for ProductVariantId {ProductVariantId}", item.UserId, product.ProductVariantId);
                }
            }
