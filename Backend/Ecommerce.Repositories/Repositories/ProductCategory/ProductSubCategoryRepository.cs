...
public class ProductSubCategoryRepsository : AbstractRepository<int, ProductSubCategory>, IProductSubCategoryRepsository
{
    ...
    public async Task<List<ProductSubCategory>> GetAllSubProductCategoryForUser(int productCategoryId)
    {
        return await _ecommerceContext.ProductSubCategory.Include(sc => sc.ProductCategory)
        .Where(sc => sc.ProductCategoryId == productCategoryId && sc.IsActive && sc.ProductCategory != null && sc.ProductCategory.IsActive)
        .ToListAsync();
    }
