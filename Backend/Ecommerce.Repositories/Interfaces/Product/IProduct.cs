...
public interface IProductRepsository : IRepository<int, Product>
{
    ...
    public Task<(List<Product> items, int totalCount)> GetVendorProduct(RequestProductFilter request, int vendorid);
    public Task<(List<Product> items, int totalCount)> GetUserProducts(RequestProductFilter request);
